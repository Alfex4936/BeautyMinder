...
//                http.httpBasic(AbstractHttpConfigurer::disable);
        ...
        http.authorizeHttpRequests(auth -> auth.requestMatchers(HttpMethod.OPTIONS, "/*").permitAll() // preflight request: ex) POST -> OPTIONS -> POST
                ...
                .requestMatchers(antMatcher("/user/forgot-password")).permitAll().requestMatchers(antMatcher("/user/sms/**")).permitAll().requestMatchers(antMatcher(HttpMethod.GET, "/user/reset-password")).permitAll().requestMatchers(antMatcher(HttpMethod.POST, "/user/reset-password")).permitAll().requestMatchers(antMatcher("/user/signup")).permitAll().requestMatchers(antMatcher("/user/signup-admin")).permitAll().requestMatchers(antMatcher("/user/sms/send")).permitAll()
...
//                    optionalRefreshToken.filter(tokenProvider::validToken).flatMap(refreshTokenService::findUserByRefreshToken).ifPresentOrElse(user -> generateNewAccessTokenAndRespond(user, request, response), () -> handleFailure(response));
        ...
        http.logout(l -> l.permitAll().logoutUrl("/logout").invalidateHttpSession(true).logoutSuccessHandler(((request, response, authentication) -> {
...
//            response.sendRedirect("/login");
        ...
        http.exceptionHandling(c -> c.defaultAuthenticationEntryPointFor(new HttpStatusEntryPoint(HttpStatus.UNAUTHORIZED), new AntPathRequestMatcher("/api/**")));
    ...
    private void saveRefreshToken(User user, String newRefreshToken) {
        ...
        RefreshToken refreshToken = refreshTokenRepository.findByUserId(user.getId()).map(entity -> {
    ...
    private void saveAccessToken(User user, String accessToken) {
        ...
        RefreshToken refreshToken = refreshTokenRepository.findByUserId(user.getId()).map(entity -> entity.update(accessToken)).orElse(new RefreshToken(user, accessToken));
    ...
    private Optional<String> getRefreshTokenFromRequest(@NotNull HttpServletRequest request) {
        ...
        return Optional.ofNullable(request.getCookies()).flatMap(cookies -> Arrays.stream(cookies).filter(c -> REFRESH_TOKEN_COOKIE_NAME.equals(c.getName())).findFirst()).map(Cookie::getValue);
    }
